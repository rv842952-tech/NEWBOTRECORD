...
-- Cycle library (shared across bots via manual BOT_ID)
CREATE TABLE IF NOT EXISTS cycle_posts (
    id            BIGSERIAL   PRIMARY KEY,
    library_id    TEXT        NOT NULL,   -- same as bot_id — set manually to share across bots
    position      INT         NOT NULL,   -- order in the cycle (1, 2, 3 ... N)
    message       TEXT,
    media_type    TEXT,
    media_file_id TEXT,
    caption       TEXT,
...
-- Cycle state (tracks recording and replay state per library)
...
CREATE INDEX IF NOT EXISTS idx_cycle_posts_lib
    ON cycle_posts (library_id, position);
...
# ─────────────────────────────────────────────────────────
# CYCLE LIBRARY  operations
# ─────────────────────────────────────────────────────────

def cycle_get_count(library_id: str) -> int:
    """Total posts in the cycle library."""
    with get_conn() as conn:
        cur = _raw(conn)
        cur.execute("SELECT COUNT(*) FROM cycle_posts WHERE library_id = %s", (library_id,))
        return cur.fetchone()[0]


def cycle_insert_post(library_id: str, position: int, message=None,
                      media_type=None, media_file_id=None, caption=None) -> int:
    """Insert one post into cycle library at given position."""
    with get_conn() as conn:
        cur = _raw(conn)
        cur.execute("""
            INSERT INTO cycle_posts
                (library_id, position, message, media_type, media_file_id, caption)
            VALUES (%s, %s, %s, %s, %s, %s)
            ON CONFLICT (library_id, position)
            DO UPDATE SET
        ...
        # update cycle_state total
        cur.execute("""
            INSERT INTO cycle_state (library_id, total_posts, last_updated)
            VALUES (%s, %s, NOW())
            ON CONFLICT (library_id)
            DO UPDATE SET
                total_posts  = (SELECT COUNT(*) FROM cycle_posts WHERE library_id = %s),
                last_updated = NOW()
        """, (library_id, 1, library_id))
        return row_id


def cycle_get_posts(library_id: str, from_position: int = 1,
                    limit: int = 500) -> list[dict]:
    """Fetch cycle posts in order starting from from_position."""
    with get_conn() as conn:
        cur = _cur(conn)
        cur.execute("""
            SELECT * FROM cycle_posts
            WHERE  library_id = %s AND position >= %s
            ORDER  BY position
            LIMIT  %s
        """, (library_id, from_position, limit))
        return [dict(r) for r in cur.fetchall()]


def cycle_get_post_at(library_id: str, position: int) -> dict | None:
    """Get a single post at a specific position."""
    with get_conn() as conn:
        cur = _cur(conn)
        cur.execute("""
            SELECT * FROM cycle_posts
...
def cycle_clear(library_id: str) -> int:
    """Delete all posts from library. Returns count deleted."""
    with get_conn() as conn:
        cur = _raw(conn)
        cur.execute("DELETE FROM cycle_posts WHERE library_id = %s", (library_id,))
        deleted = cur.rowcount
        cur.execute("""
            INSERT INTO cycle_state (library_id, total_posts, last_updated)
            VALUES (%s, 0, NOW())
            ON CONFLICT (library_id)
            DO UPDATE SET total_posts = 0, last_updated = NOW()
        """, (library_id,))
        return deleted


def cycle_preview(library_id: str, count: int = 5) -> list[dict]:
    """Get first N and last N posts for preview."""
    with get_conn() as conn:
        cur = _cur(conn)
        cur.execute("""
            (SELECT * FROM cycle_posts WHERE library_id = %s ORDER BY position LIMIT %s)
            UNION ALL
            (SELECT * FROM cycle_posts WHERE library_id = %s ORDER BY position DESC LIMIT %s)
        """, (library_id, count, library_id, count))
        return [dict(r) for r in cur.fetchall()]


def cycle_reorder(library_id: str, post_id: int, new_position: int) -> bool:
    """Move a post to a new position, shifting others."""
    with get_conn() as conn:
        cur = _raw(conn)
        # get current position
        cur.execute("SELECT position FROM cycle_posts WHERE id = %s AND library_id = %s",
                    (post_id, library_id))
        row = cur.fetchone()
        if not row:
            return False
        ...
        if old_pos == new_position:
            return True
        # shift posts between old and new position
        if new_position > old_pos:
            cur.execute("""
                UPDATE cycle_posts SET position = position - 1
                WHERE library_id = %s AND position > %s AND position <= %s
            """, (library_id, old_pos, new_position))
        else:
            cur.execute("""
                UPDATE cycle_posts SET position = position + 1
                WHERE library_id = %s AND position >= %s AND position < %s
            """, (library_id, new_position, old_pos))
        cur.execute("UPDATE cycle_posts SET position = %s WHERE id = %s AND library_id = %s",
                    (new_position, post_id, library_id))
        return True
