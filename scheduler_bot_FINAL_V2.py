...
# ── Record Mode ───────────────────────────────────────────────────────────────
# When ON: every post collected is ALSO saved to cycle library in DB.
# library_id = BOT_ID — set same BOT_ID on any bot to share the same library.
_record_mode: bool = False
_record_session: list = []    # buffer of posts collected in current /record session
_record_next_pos: int = 1     # next position to assign (loaded from DB on start)
...
def get_mode_keyboard():
    ...
    record_label = "🔴 Record Mode: ON"  if _record_mode      else "⚪ Record Mode: OFF"
...
def _save_to_library_if_recording(content: dict):
    """If record mode is ON, save this post to cycle library."""
    global _record_next_pos
    if not _record_mode or not content:
        return
    if not BOT_ID:
        return
    try:
        db.cycle_insert_post(
            BOT_ID, _record_next_pos,
            content.get('message'), content.get('media_type'),
            content.get('media_file_id'), content.get('caption'))
        _record_next_pos += 1
        logger.info(f"🎙 Recorded post #{_record_next_pos - 1} to library")
    except Exception as e:
        logger.error(f"Failed to save post to cycle library: {e}")
...
async def _schedule_bulk(update, session):
    ...
    for i, p in enumerate(posts):
        ...
        _save_to_library_if_recording(p)
        info.append((pid, t))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "Record Mode" in text: await toggle_record_mode(update, context); return

    # ── Active recording session (/record bulk collect) ───────────────────────
    if sess.get('mode') == 'recording' and sess.get('step') == 'collect':
        if "DONE" in text or "Save to Library" in text:
            # save buffered posts to DB
            if not _record_session:
                ...
                total = db.cycle_get_count(BOT_ID)
                await update.message.reply_text(
                    f"✅ <b>Saved {saved} posts to library!</b>\n"
                    f"📚 Total in library: <b>{total}</b>",
                    reply_markup=get_mode_keyboard(), parse_mode='HTML')
            user_sessions[uid] = {'mode': None, 'step': 'choose_mode'}
            return
        # collect the post
        content_data = extract_content(update.message)
        if content_data:
            _record_session.append(content_data)
            count = len(_record_session)
            await update.message.reply_text(
                f"📥 Post #{count} buffered. Keep sending or press Done.",
                reply_markup=ReplyKeyboardMarkup(
                    [[KeyboardButton("✅ DONE — Save to Library")],
                     [KeyboardButton("❌ Cancel")]],
                    resize_keyboard=True))
        return
...
def _load_record_pos():
    """Load next record position from DB on startup."""
    global _record_next_pos
    try:
        count = db.cycle_get_count(BOT_ID)
        _record_next_pos = count + 1
    except Exception as e:
        logger.warning(f"Could not load cycle count: {e}")
        _record_next_pos = 1


async def toggle_record_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Toggle record mode ON/OFF via keyboard button or /recordmode command."""
    if not _is_admin(update): return
    global _record_mode
    _record_mode = not _record_mode
    status = "🔴 ON" if _record_mode else "⚪ OFF"
    count = db.cycle_get_count(BOT_ID)
    msg = (f"🎙 <b>Record Mode: {status}</b>\n\n"
           f"📚 Library: <b>{count} posts</b>\n\n"
           + ("Every post you schedule will also be saved to the cycle library."
              if _record_mode else
              "Scheduling works normally. Library is untouched."))
...
async def record_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /record — Start a bulk recording session.
    Bot collects posts one by one until user sends DONE.
    Nothing gets scheduled — posts go only to the library.
    """
    if not _is_admin(update): return
    global _record_session
    uid = update.effective_user.id
    _record_session = []
    user_sessions[uid] = {'mode': 'recording', 'step': 'collect'}
    count = db.cycle_get_count(BOT_ID)
    await update.message.reply_text(
        f"🎙 <b>Record Session Started</b>\n\n"
        f"📚 Current library: <b>{count} posts</b>\n\n"
        f"Send your posts one by one (text, photo, video, document).\n"
        f"Send <b>DONE</b> when finished to save them all.",
        reply_markup=ReplyKeyboardMarkup(
            [[KeyboardButton("✅ DONE — Save to Library")],
             [KeyboardButton("❌ Cancel")]],
            resize_keyboard=True),
...
async def library_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/librarystatus — Show cycle library info."""
    if not _is_admin(update): return
    count = db.cycle_get_count(BOT_ID)
    record_status = "🔴 ON" if _record_mode else "⚪ OFF"
    await update.message.reply_text(
        f"📚 <b>Cycle Library Status</b>\n\n"
        f"🎙 Record Mode: <b>{record_status}</b>\n"
        f"📝 Total posts in library: <b>{count}</b>\n"
        f"🆔 Library ID: <code>{BOT_ID}</code>\n\n"
        f"Any bot with <code>BOT_ID={BOT_ID}</code> shares this library.",
        reply_markup=get_mode_keyboard(), parse_mode='HTML')


async def library_preview_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/librarypreview — Show first 5 and last 5 posts."""
    if not _is_admin(update): return
    count = db.cycle_get_count(BOT_ID)
    if count == 0:
        await update.message.reply_text("📚 Library is empty. Use /record to add posts.",
                                        reply_markup=get_mode_keyboard())
        return
    posts = db.cycle_get_posts(BOT_ID, from_position=1, limit=5)
    last_posts = db.cycle_get_posts(BOT_ID, from_position=max(1, count - 4), limit=5)
    resp = f"📚 <b>Library Preview</b> ({count} total)\n\n<b>First 5:</b>\n"
    for p in posts:
        preview = (p.get('message') or p.get('caption') or f"[{p.get('media_type','media')}]")[:50]
        resp += f"  #{p['position']} — {preview}\n"
    if count > 5:
        resp += f"\n<b>Last 5:</b>\n"
        for p in last_posts:
            preview = (p.get('message') or p.get('caption') or f"[{p.get('media_type','media')}]")[:50]
...
async def clear_library_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/clearlibrary — Delete all posts from cycle library (with confirmation)."""
    if not _is_admin(update): return
    uid = update.effective_user.id
    count = db.cycle_get_count(BOT_ID)
    if count == 0:
        await update.message.reply_text("📚 Library is already empty.",
                                        reply_markup=get_mode_keyboard())
        return
    user_sessions[uid] = {'mode': 'clear_library_confirm', 'step': 'confirm'}
    await update.message.reply_text(
        f"⚠️ <b>Clear Library?</b>\n\n"
        f"This will permanently delete all <b>{count} posts</b> from the library.\n"
...
async def set_position_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/setposition X — Set replay start position manually."""
    if not _is_admin(update): return
    args = context.args
    if not args:
        await update.message.reply_text(
            "Usage: /setposition <number>\nExample: /setposition 150",
            reply_markup=get_mode_keyboard())
        return
    try:
        pos = int(args[0])
        ...
        if pos < 1 or pos > count:
            await update.message.reply_text(
                f"❌ Position must be between 1 and {count}.",
                reply_markup=get_mode_keyboard())
            return
        # Store in bot_settings
        _save_setting('replay_position', str(pos))
        await update.message.reply_text(
            f"✅ Replay position set to <b>#{pos}</b>\n"
            f"Next replay will start from post #{pos}.",
            reply_markup=get_mode_keyboard(), parse_mode='HTML')
    except ValueError:
        await update.message.reply_text("❌ Invalid number.", reply_markup=get_mode_keyboard())
...
def main():
    ...
    _load_record_pos()   # load cycle library position
    ...
    app.add_handler(CommandHandler("recordmode",     toggle_record_mode))
    app.add_handler(CommandHandler("librarystatus",  library_status_command))
    app.add_handler(CommandHandler("librarypreview", library_preview_command))
    app.add_handler(CommandHandler("clearlibrary",   clear_library_command))
    app.add_handler(CommandHandler("setposition",    set_position_command))
